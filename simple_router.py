...
class node():
    ...
    def printNextNodes(self):
        ...
        for i in self.connections:
            ...
            print("id: " + otherNode.id + " lat: " + otherNode.lat + " lon: " + otherNode.lon + " distance: " + str(
                calcDistance(self, otherNode)))

    def raiseKnowledge(self, prefNode, destNode):
        print(
            "going to know more about: " + self.id + " Score: " + str(self.getScore()) + " prfNode was: " + prefNode.id)
...
class routingDb():
    ...
    def printStatus(self):
        ...
        for i in self.listOfNodes:
            print("Node " + str(i) + " is in da DB it has: " + str(
                self.listOfNodes[i].numOfConnections()) + " Connection(s)")
...
def run_simple_router(filename):
    db = routingDb()
    print(readDb(open(filename, "rb"), db))
    db.cleanUp()
    path = startRouting(db)
    return path

if __name__ == "__main__":
    db = routingDb()
    print(readDb(open(filepath, "rb"), db))
    db.cleanUp()
    startRouting(db)
